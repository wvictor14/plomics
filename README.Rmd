...
For creating a sample sheet out of sample IDs, we need a list of file paths corresponding to each
sample. `makeSampleSheet` takes a vector of sample IDs and will find all associated idats and 
return their corresponding file paths in a data frame along with other sample variables present in 
the master sample sheet. 

Basically searches the 'Sample_Name' column for matches based on the provided `samples` vector. Non-
exact matches are tolerated, so for example PM306 will return PM306 and PM306_d.
